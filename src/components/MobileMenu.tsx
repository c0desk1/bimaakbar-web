...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, currentPath, onLinkClick }) => {
  ...
  return (
    ...
    >
      <div className="container mx-auto flex flex-col items-center justify-center h-full gap-8">
        {LINKS.map((link) => (
          <a key={link.HREF} href={link.HREF} className={getMobileLinkClass(link.HREF)} onClick={onLinkClick}>
            {link.TEXT}
          </a>
        ))}

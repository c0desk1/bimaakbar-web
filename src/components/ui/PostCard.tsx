    ...
    <a href={`/blog/${post.slug}`}
      ...
      <div className="flex flex-col flex-grow h-full">
        ...
        <div className="flex justify-between items-center mb-2">
          <div className="flex-1 text-sm uppercase text-[var(--color-muted)]">
            {post.date ? formatDate(post.date) : "Tanpa tanggal"}
          </div>
          <div className="flex-shrink-0 text-sm capitalize px-2 py-0.5 rounded-xl border border-[var(--color-border)]">
            {post.category}
          </div>
        </div>

        <div className="flex items-center font-semibold text-[var(--color-fg)] line-clamp-2 mb-2">
          {post.featured && (
        ...
        <div className="text-sm text-[var(--color-muted)] line-clamp-2">
          {post.description}
        </div>
        ...
        {post.tags.length > 0 && (
          <ul className="flex flex-wrap mt-auto gap-1">
            {[...new Set(post.tags)].map((tag) => (
              <li
                key={tag}
                className="text-center text-xs uppercase px-2 py-1 rounded-xl text-[var(--color-fg)] border border-[var(--color-border)]">
                # {tag}
              </li>
            ))}
          </ul>

...
const googleSheet = import.meta.env.PUBLIC_GOOGLE_SHEET_URL;
      ...
      ? data
          ...
          .map((item): Blog => {
            ...
            return {
              ...
              tags,
              author: item.author || SITE.title,
              date: parseDate(item.date) || now,
              lastModified: parseDate(item.lastModified),
              cover: !item.cover || item.cover.toLowerCase() === item.title ? null : item.cover,

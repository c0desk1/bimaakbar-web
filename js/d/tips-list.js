function loadPostsTips() {
  ...
  fetch('https://opensheet.elk.sh/1l7d-9BwbbH5I-jj_Uw0-eA2mkJEWV_yWyt9RquhM_XY/tips-trik')
    .then(function(res) {
      if (!res.ok) throw new Error('Gagal fetch Tips & Trik');
      return res.json();
    })
    .then(function(data) {
      ...
      filteredPosts.forEach(post => {
        ...
        postEl.innerHTML =
          `<a href="${post.url}">
            <img src="${post.thumbnail}" alt="${post.title}" loading="lazy" onerror="this.onerror=null;this.src='/assets/logo.png';">
            <h3>${post.title}</h3>
